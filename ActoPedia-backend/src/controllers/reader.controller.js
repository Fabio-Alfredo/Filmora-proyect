import {getReaders, saveReader, deleteReader} from "../service/reader.service.js";
...
export const deleteReaderController = async (req, res) => {
    try{
        const token = req.headers.authorization.split(" ")[1];
        if (!token) throw new HttpError(401, "Token required");
        ...
        const response = await deleteReader(actorId, decoded.id);
        res.send(response);
    }catch(err){
        handleHttpError(res, err.message, err);
    }
